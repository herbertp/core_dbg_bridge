...
entity ps7_stub is
	...
	--
	s_gp0_aclk	: in std_ulogic				:= '0';
	s_gp0_areset_n  : out std_ulogic;
	--	read address
	s_gp0_arid	: in std_logic_vector (5 downto 0)	:= (others => '0');
	s_gp0_araddr	: in std_logic_vector (31 downto 0)	:= (others => '0');
	s_gp0_arburst	: in std_logic_vector (1 downto 0)	:= "01";
	s_gp0_arlen	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_arsize	: in std_logic_vector (1 downto 0)	:= "10";
	s_gp0_arlock	: in std_logic_vector (1 downto 0)	:= (others => '0');
	s_gp0_arprot	: in std_logic_vector (2 downto 0)	:= (others => '0');
	s_gp0_arcache	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_arqos	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_arvalid	: in std_ulogic				:= '0';
	s_gp0_arready	: out std_ulogic;
	--	read data
	s_gp0_rid	: out std_logic_vector (5 downto 0);
	s_gp0_rdata	: out std_logic_vector (31 downto 0);
	s_gp0_rlast	: out std_ulogic;
	s_gp0_rresp	: out std_logic_vector (1 downto 0);
	s_gp0_rvalid	: out std_ulogic;
	s_gp0_rready	: in std_ulogic				:= '0';
	--	write address
	s_gp0_awid	: in std_logic_vector (5 downto 0)	:= (others => '0');
	s_gp0_awaddr	: in std_logic_vector (31 downto 0)	:= (others => '0');
	s_gp0_awburst	: in std_logic_vector (1 downto 0)	:= "01";
	s_gp0_awlen	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_awsize	: in std_logic_vector (1 downto 0)	:= "10";
	s_gp0_awlock	: in std_logic_vector (1 downto 0)	:= (others => '0');
	s_gp0_awprot	: in std_logic_vector (2 downto 0)	:= (others => '0');
	s_gp0_awcache	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_awqos	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_awvalid	: in std_ulogic				:= '0';
	s_gp0_awready	: out std_ulogic;
	--	write data
	s_gp0_wid	: in std_logic_vector (5 downto 0)	:= (others => '0');
	s_gp0_wdata	: in std_logic_vector (31 downto 0)	:= (others => '0');
	s_gp0_wstrb	: in std_logic_vector (3 downto 0)	:= (others => '0');
	s_gp0_wlast	: in std_ulogic				:= '0';
	s_gp0_wvalid	: in std_ulogic				:= '0';
	s_gp0_wready	: out std_ulogic			:= '0';
	--	write response
	s_gp0_bid	: out std_logic_vector (5 downto 0);
	s_gp0_bresp	: out std_logic_vector (1 downto 0);
	s_gp0_bvalid	: out std_ulogic;
	s_gp0_bready	: in std_ulogic				:= '0';
	--
	uart0_tx	: out std_ulogic;
	uart0_rx	: in std_ulogic				:= '1'
    );
end entity ps7_stub;
...
begin
	...
	EMIOUART0TX			=> uart0_tx,		-- out std_ulogic;
	...
	SAXIGP0ARESETN			=> s_gp0_areset_n,	-- out std_ulogic;
	SAXIGP0ARREADY			=> s_gp0_arready,	-- out std_ulogic;
	SAXIGP0AWREADY			=> s_gp0_awready,	-- out std_ulogic;
	SAXIGP0BID			=> s_gp0_bid,		-- out std_logic_vector (5 downto 0);
	SAXIGP0BRESP			=> s_gp0_bresp,	-- out std_logic_vector (1 downto 0);
	SAXIGP0BVALID			=> s_gp0_bvalid,	-- out std_ulogic;
	SAXIGP0RDATA			=> s_gp0_rdata,	-- out std_logic_vector (31 downto 0);
	SAXIGP0RID			=> s_gp0_rid,		-- out std_logic_vector (5 downto 0);
	SAXIGP0RLAST			=> s_gp0_rlast,	-- out std_ulogic;
	SAXIGP0RRESP			=> s_gp0_rresp,	-- out std_logic_vector (1 downto 0);
	SAXIGP0RVALID			=> s_gp0_rvalid,	-- out std_ulogic;
	SAXIGP0WREADY			=> s_gp0_wready,	-- out std_ulogic;
	...
	EMIOUART0RX			=> uart0_rx,		-- in std_ulogic;
	...
	SAXIGP0ACLK			=> s_gp0_aclk,		-- in std_ulogic;
	SAXIGP0ARADDR			=> s_gp0_araddr,	-- in std_logic_vector (31 downto 0);
	SAXIGP0ARBURST			=> s_gp0_arburst,	-- in std_logic_vector (1 downto 0);
	SAXIGP0ARCACHE			=> s_gp0_arcache,	-- in std_logic_vector (3 downto 0);
	SAXIGP0ARID			=> s_gp0_arid,		-- in std_logic_vector (5 downto 0);
	SAXIGP0ARLEN			=> s_gp0_arlen,	-- in std_logic_vector (3 downto 0);
	SAXIGP0ARLOCK			=> s_gp0_arlock,	-- in std_logic_vector (1 downto 0);
	SAXIGP0ARPROT			=> s_gp0_arprot,	-- in std_logic_vector (2 downto 0);
	SAXIGP0ARQOS			=> s_gp0_arqos,	-- in std_logic_vector (3 downto 0);
	SAXIGP0ARSIZE			=> s_gp0_arsize,	-- in std_logic_vector (1 downto 0);
	SAXIGP0ARVALID			=> s_gp0_arvalid,	-- in std_ulogic;
	SAXIGP0AWADDR			=> s_gp0_awaddr,	-- in std_logic_vector (31 downto 0);
	SAXIGP0AWBURST			=> s_gp0_awburst,	-- in std_logic_vector (1 downto 0);
	SAXIGP0AWCACHE			=> s_gp0_awcache,	-- in std_logic_vector (3 downto 0);
	SAXIGP0AWID			=> s_gp0_awid,		-- in std_logic_vector (5 downto 0);
	SAXIGP0AWLEN			=> s_gp0_awlen,	-- in std_logic_vector (3 downto 0);
	SAXIGP0AWLOCK			=> s_gp0_awlock,	-- in std_logic_vector (1 downto 0);
	SAXIGP0AWPROT			=> s_gp0_awprot,	-- in std_logic_vector (2 downto 0);
	SAXIGP0AWQOS			=> s_gp0_awqos,	-- in std_logic_vector (3 downto 0);
	SAXIGP0AWSIZE			=> s_gp0_awsize,	-- in std_logic_vector (1 downto 0);
	SAXIGP0AWVALID			=> s_gp0_awvalid,	-- in std_ulogic;
	SAXIGP0BREADY			=> s_gp0_bready,	-- in std_ulogic;
	SAXIGP0RREADY			=> s_gp0_rready,	-- in std_ulogic;
	SAXIGP0WDATA			=> s_gp0_wdata,	-- in std_logic_vector (31 downto 0);
	SAXIGP0WID			=> s_gp0_wid,		-- in std_logic_vector (5 downto 0);
	SAXIGP0WLAST			=> s_gp0_wlast,	-- in std_ulogic;
	SAXIGP0WSTRB			=> s_gp0_wstrb,	-- in std_logic_vector (3 downto 0);
	SAXIGP0WVALID			=> s_gp0_wvalid,	-- in std_ulogic;
